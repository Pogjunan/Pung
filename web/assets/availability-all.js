...
function recordState(record, frameIndex) {
  if (frameIndex < state.sourceOffset) {
    return "NO_SOURCE";
  }
  return containsFrame(record.shifted_on_day_runs, frameIndex)
    ? "ON"
    : "OFF";
}
...
function popupContent(record, frameIndex, currentState) {
  ...
  return `
    <div class="availability-popup">
      ...
      <b>현재 날짜:</b> ${displayDate(frameIndex)}<br>
      <b>현재 상태:</b>
      ...
      <hr>
      <small>
        EVER ON 검증은 전체 원본 기간에서 각 station이
        최소 한 날짜 이상 ON인지 확인한 결과입니다.
      </small>
    </div>
  `;
}
...
async function initialize() {
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `daily availability request failed: ${response.status}`,
      );
    }
